...
def voxel_select_point(filtered_points, filter_type,hash_table_point, hash_index, ):
...
def voxel_grid_sample(point_cloud, grid_size, container_num, filter_type='random'):
    ...
    for point in point_cloud:
        ...
        if not hash_index in hash_table_index:   # new hash index
            hash_table_index[hash_index]=h
            hash_table_point[hash_index]=[point]
        elif h==hash_table_index[hash_index]:    # the same h and hash index
            hash_table_point[hash_index].append(point)
        else:                                    # the same hash index, but h is not the same
            filtered_points = voxel_select_point(filtered_points,filter_type,hash_table_point,hash_index)
            hash_table_index[hash_index]=h
            hash_table_point[hash_index]=[point]
...
if __name__ == "__main__":
    point_cloud_original = np.loadtxt(r'F:\dataset\modelnet40_normal_resampled\chair\chair_0001.txt', dtype="float64", delimiter=",")[:, 0:3]
