...
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    if len(point_indices) > leaf_size:
        ...
        # 作业1
        # 屏蔽开始
        # step1 the first node and its left leaf and right leaf
        median_left_leaf_index = math.ceil(point_indices_sorted.shape[0] / 2) - 1
        left_leaf_index = point_indices_sorted[median_left_leaf_index]
        left_leaf_value = db[left_leaf_index, axis]
        median_right_leaf_index = median_left_leaf_index + 1
        right_leaf_index = point_indices_sorted[median_right_leaf_index]
        right_leaf_value = db[right_leaf_index, axis]
        # step2 build the kdtree recursively
        root.value = (left_leaf_value+right_leaf_value)*0.5
        root.left = kdtree_recursive_build(root.left, db, point_indices_sorted[0:median_right_leaf_index], axis_round_robin(axis, dim=db.shape[1]), leaf_size)
        root.right = kdtree_recursive_build(root.right, db, point_indices_sorted[median_right_leaf_index:], axis_round_robin(axis, dim=db.shape[1]), leaf_size)
        # # 屏蔽结束
    return root
