...
def octree_recursive_build(root, db, center, extent, point_indices, leaf_size, min_extent):
    ...
    if root is None:
        root = Octant([None for i in range(8)], center,
                      extent, point_indices, is_leaf=True)
    ...
    if len(point_indices) <= leaf_size or extent <= min_extent:
        ...
        for i in range(8):
            ...
            child_center = np.asarray(
                [child_center_x, child_center_y, child_center_z])
            root.children[i] = octree_recursive_build(
                root.children[i], db, child_center, child_extent, children_point_indices[i], leaf_size, min_extent)
            # 屏蔽结束
    return root
